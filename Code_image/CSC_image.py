...
- The goal of this solver is to find the common filters (or kernel), the codes for each image
in the dataset and a reconstruction for the dataset.
...
def learn_conv_sparse_coder(b, size_kernel, max_it, tol, beta=np.float64(1.0)):
    """
    ...
    for i in range(max_it):
        ...
        for i_z in range(max_it_z):
            ...
            u_Z[1] = np.multiply(np.maximum(0, 1 - np.divide(theta*beta, np.abs(u))), u)
